class Creature{
  ...
  void display(){
    ...
    ellipse(pos.x,pos.y, 2*health+4, 2*health+4);
  }
  void update(){
    display();
    ...
    float[] inputs = {food_board[currentFoodx][currentFoody],
                      health,
  ...
  void move(PVector _dir){
    ...
    PVector b = pos.copy();
    pos.add(dir);
    if(pos.x > width){pos = b.copy();}
    if(pos.y > height){pos = b.copy();}
    if(pos.x < 0){pos = b.copy();}
    if(pos.y < 0){pos = b.copy();}
  }
}